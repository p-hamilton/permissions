...
public class SyncText {
    ...
    public void update(String newText){
        ...
        LinkedList<diff_match_patch.Patch> patches = DMP.patch_make(text, newText);

        if(patches.size() > 0){
            String patchString = DMP.patch_toText(patches);
    ...
    private void processPatch(SyncTextPatch patch){
        if(patch == null) throw new IllegalArgumentException("null patch");
        ...
        LinkedList<diff_match_patch.Patch> remotePatch = new LinkedList<>(DMP.patch_fromText(patch.getPatch()));
        Object[] results = DMP.patch_apply(remotePatch, this.text);
        //TODO: check results
